...
//计算灰度信息
void ImageProcesser::calculateGrayInfo(QImage grayimage, double*grayInfo){
...
void ImageProcesser::showHistogram(QImage grayimage){
    ...
    GrayInfo->setText(tr("平均灰度：%1  中值灰度：%2  标准差：%3  像素总数：%4")
                       .arg(showinggrayInfo[0]).arg(showinggrayInfo[1]).arg(showinggrayInfo[2]).arg(showinggrayInfo[3]));
